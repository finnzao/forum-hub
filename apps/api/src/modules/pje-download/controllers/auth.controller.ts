...
export async function authRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: { sessionId: string; profileIndex: number } }>('/profile', async (request, reply) => {
    ...
    if (result.error) {
      ...
      return reply.status(sc).send({
        success: false,
        error: { code: isExpired ? 'SESSION_EXPIRED' : 'PROFILE_ERROR', message: isExpired ? 'Sessão PJE expirada. Faça login novamente.' : result.error, statusCode: sc },
      });
    }
  ...
  fastify.get<{ Querystring: { sessionId: string } }>('/debug-profiles', async (request, reply) => {
    const sessionId = (request.query as any).sessionId;
