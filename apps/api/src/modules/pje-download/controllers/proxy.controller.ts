    ...
    async (request, reply) => {
      ...
      if (!entry || Date.now() > entry.expiresAt) {
        ...
        return reply.status(410).send({
          success: false,
          error: { code: 'URL_EXPIRED', message: 'URL expirada. Solicite novamente.', statusCode: 410 },
        });
      }
      ...
      try {
        ...
        if (!upstream.ok || !upstream.body) {
          return reply.status(502).send({
            success: false,
            error: { code: 'UPSTREAM_ERROR', message: 'Falha ao obter PDF do PJE.', statusCode: 502 },
          });
        }
        ...
        return reply.status(502).send({
          success: false,
          error: { code: 'PROXY_ERROR', message: 'Erro ao fazer proxy do download.', statusCode: 502 },
        });
      } finally {
        urlCache.delete(request.params.token);
      }
