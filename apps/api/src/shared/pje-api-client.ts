...
export function serializeAllCookies(cookies: Record<string, string>): string {
  return serializeCookies(cookies);
}

export function serializePjeCookies(cookies: Record<string, string>): string {
  return serializeCookies(cookies, 'pje.tjba.jus.br');
}
...
export function buildPjeHeaders(session: PjeSession): Record<string, string> {
  return {
    ...
    'X-pje-cookies': serializeAllCookies(session.cookies),
