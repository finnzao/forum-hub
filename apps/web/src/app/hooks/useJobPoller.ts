...
export function useJobPoller(sessionId?: string) {
  ...
  const criarNovoJob = useCallback(async (
    params: ParametrosDownload,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (jobsRef.current.some((j) => isJobActive(j.status))) {
        recarregarJobs();
        carregarProgresso();
      }
    }, 10_000);
  ...
  return {
    jobs, mapaProgresso,
    criarNovoJob, cancelarJobById, recarregarJobs,
  };
}
