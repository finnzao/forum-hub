// ============================================================
// apps/web/src/app/componentes/pje-download/PainelLogs.tsx
// Painel de logs colapsável (desenvolvimento)
// FIX: Substituído <button> aninhado por <div role="button">
// ============================================================
...
export function PainelLogs({ logs, onLimpar }: Props) {
  ...
  return (
    <div className="bg-slate-900 border-t-2 border-slate-700">
      {/* Toggle — usa <div> em vez de <button> para evitar aninhamento */}
      <div
        role="button"
        tabIndex={0}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {logs.length > 0 && (
            <span className="px-1.5 py-0.5 bg-slate-800 text-slate-500 text-[10px] font-mono">
              {logs.length}
            </span>
          )}
        </div>
      ...
      {aberto && (
        <div
          ref={scrollRef}
          className="max-h-48 overflow-y-auto px-4 pb-3 font-mono text-[11px] leading-relaxed"
        >
          {logs.length === 0 ? (
            ...
            logs.map((log) => (
              <div key={log.id} className="flex gap-2 py-0.5">
                ...
                <span className={`flex-shrink-0 font-bold ${NIVEL_CORES[log.nivel] || 'text-slate-400'}`}>
                ...
                {log.dados && (
                  <details className="inline">
                    <summary className="text-slate-600 cursor-pointer hover:text-slate-400">▸</summary>
                    <pre className="text-slate-600 whitespace-pre-wrap mt-1 ml-2">
