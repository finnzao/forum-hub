// ============================================================
// apps/web/src/app/componentes/pje-download/CardJob.tsx
// Card de um job de download com progresso e ações
// ============================================================
...
import {
  ChevronDown, ChevronUp, X, Lock, Download,
  CheckCircle, AlertCircle, Clock, Loader2,
} from 'lucide-react';
import type { DownloadJobResponse, PJEDownloadProgress } from 'shared';
import { isJobActive, STATUS_LABELS, STATUS_COLORS } from './helpers';
...
export function CardJob({ job, progresso, expandido, onAlternarExpansao, onCancelar, onAbrir2FA }: Props) {
  ...
  const cor = STATUS_COLORS[job.status] || STATUS_COLORS.pending;
  ...
  const StatusIcon = () => {
    switch (job.status) {
      ...
      default: return ativo ? <Loader2 size={14} className="animate-spin text-blue-600" /> : <Clock size={14} className="text-slate-400" />;
    }
  };
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`px-1.5 py-0.5 text-[10px] font-bold ${cor.bg} ${cor.text}`}>
              {label}
            </span>
          </div>
          <p className="text-xs text-slate-600 truncate mt-0.5">
            {progresso?.message || `Modo: ${job.mode}`}
          </p>
        </div>
        ...
        <div className="px-3 pb-2">
          ...
          <div className="flex justify-between mt-1">
            ...
            <span className="text-[10px] text-slate-400">
              {job.successCount}/{job.totalProcesses || '?'}
            </span>
          </div>
        </div>
      )}
          ...
          <div className="flex gap-2 pt-1">
            {job.status === 'awaiting_2fa' && (
              <button
                type="button"
                onClick={onAbrir2FA}
                className="flex-1 px-2 py-1.5 text-xs font-bold bg-amber-100 text-amber-800 hover:bg-amber-200"
              >
                Inserir código 2FA
              </button>
            )}
            {ativo && (
              <button
                type="button"
                onClick={onCancelar}
                className="flex-1 px-2 py-1.5 text-xs font-bold border-2 border-red-200 text-red-600 hover:bg-red-50"
              >
                Cancelar
              </button>
            )}
